...
const regs = {
    ...
    nameData: new RegExp(/^[1-9]{2,}$/),
    famalyRegEx: new RegExp(/^[a-zA-Z]{2,}$/),
...
let checkFamaly = (userFamaly) => {
    return regs.famalyRegEx.test(userFamaly.value);
}
let checkData = (userData) => {
    return regs.nameData.test(userData.value);
}
...
function checkRegistrationForm() {
    ...
    let userData = inputsReg.find(el => el.type == "text");
    let userFamaly = inputsReg.find(el => el.type == "text");

    let resEmail = checkEmail(email);
    let resPassword = checkPassword(password);
    let resName = checkName(userName);
    let resfamaly = checkFamaly(userFamaly);
    let resdata = checkData(userData);
    ...
    if (!resfamaly) {
        userName.value = "Famaly is not valid";
    }
    if (!resdata) {
        userName.value = "Data is not valid";
    }
    isClicked = false;
}
