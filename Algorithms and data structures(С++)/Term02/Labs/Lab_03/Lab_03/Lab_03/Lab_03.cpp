...
void printDistance(char name[], int distance[], int n);
...
void printDistance(char name[], int distance[], int n)
{
    ...
    for (int i = 0; i < n; i++)
        std::cout << "Кратчайший путь до вершины " << name[i] << " равен = " << distance[i] << std::endl;
}
...
void dijkstra(int graph[V][V], int src)
{
    ...
    printDistance(nameSymbol, distance, V);
}